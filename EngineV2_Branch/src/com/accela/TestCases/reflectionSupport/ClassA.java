...
public class ClassA
{
	...
	private void init()
	{
		...
		ClassAB.addToSet(mapB);
		for(int i=0;i<SIZE;i++)
		{
			Integer key=rand.nextInt();
			ClassB val=new ClassB(this);
			mapB.put(key, val);
			ClassAB.addToSet(key);
			ClassAB.addToSet(val);
		}
		classB=new ClassB(this);
		ClassAB.addToSet(classB);
		...
		iWrap=rand.nextInt();
		ClassAB.addToSet(iWrap);
		boolWrap=rand.nextBoolean();
		ClassAB.addToSet(boolWrap);
		dWrap=rand.nextDouble();
		ClassAB.addToSet(dWrap);
		fWrap=rand.nextFloat();
		ClassAB.addToSet(fWrap);
		lWrap=rand.nextLong();
		ClassAB.addToSet(lWrap);
		cWrap=(char)rand.nextInt();
		ClassAB.addToSet(cWrap);
		sWrap=(short)rand.nextInt();
		ClassAB.addToSet(sWrap);
		bWrap=(byte)rand.nextInt();
		ClassAB.addToSet(bWrap);
		
		str=rand.nextGaussian()+"";
		ClassAB.addToSet(str);
		
		abc=ABC.values()[rand.nextInt(ABC.values().length)];
		ClassAB.addToSet(abc);
		classBStatic=new ClassB(this, false);
